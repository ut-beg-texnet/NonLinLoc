...
|    ``ttimeFileRoot`` (*string*) full or relative path and file *root*
  ...
  Optionally, can be a comma-separated set (no spaces) of ``ttimeFileRoot`` values; NLLoc uses the first of these for which a valid time grid is found for each station and phase needed for location, allowing, e.g., the use of 3D model grids near a station and 1D model grids (2D time grids) at larger distances.
|    ``outputFileRoot`` (*string*) full or relative path and file *root*
